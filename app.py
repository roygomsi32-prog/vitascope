...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tab2:
        ...
        with col_a:
            fig_sc = px.scatter(df, x="heures_sommeil", y="moyenne_academique", color="stress",
                                ...
                                color_continuous_scale="RdYlGn_r", trendline="ols")
    ...
    with tab4:
        ...
        st.dataframe(stats_df.style.background_gradient(cmap="Greens", subset=["Moyenne"]),
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if len(x_vars) < 2:
        ...
        coef_df = pd.DataFrame({
            ...
            "Importance relative (%)": (np.abs(reg.coef_) / np.abs(reg.coef_).sum() * 100).round(1),
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        df_coords[color_var] = df.loc[df_acp.index, color_var].values
    ...
    with tab2:
        if n_comp >= 3:
            ...
            df_3d[color_var] = df.loc[df_acp.index, color_var].values
    ...
    with tab3:
        ...
        st.dataframe(loadings_df.style.background_gradient(cmap="RdYlGn", axis=None), use_container_width=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    df_cl["Cluster"] = [f"Cluster {l}" if l >= 0 else "Bruit" for l in labels]

    n_clusters = len(set(labels[labels >= 0]))
    try:
        sil = silhouette_score(X_scaled[labels >= 0], labels[labels >= 0]) if n_clusters > 1 else 0
        db = davies_bouldin_score(X_scaled[labels >= 0], labels[labels >= 0]) if n_clusters > 1 else 0
    except Exception:
        sil, db = 0, 0
    ...
    profils_display["N"] = df_cl["Cluster"].value_counts()
    st.dataframe(profils_display.style.background_gradient(cmap="YlGn"), use_container_width=True)
